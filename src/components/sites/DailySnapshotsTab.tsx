...
import { Site, DailySnapshot } from '../../types';
import {
  Calendar,
  Clock,
  AlertTriangle,
...
export const DailySnapshotsTab: React.FC<DailySnapshotsTabProps> = ({
  ...
  const defaultDate = initialSelectedDate || (snapshots.length > 0 ? snapshots[snapshots.length - 1].date : '');
    ...
    <div className="space-y-5 animate-in fade-in duration-150">
      ...
      <div className="bg-blue-50/70 border border-blue-200 rounded-lg p-4 shadow-sm flex items-start gap-3 text-xs text-slate-700 leading-relaxed">
        ...
        <div>
          ...
          每日打点按北京时间 <strong>00:00:00 至 23:59:59</strong> 进行标准日切。跨日事件按实际发生时段精确切分；计划内维护（已申报工单）不计入等效中断时长。点击下方日历列表或趋势图上的任意日期，可即时展开当日事件快照与因果追溯。
        </div>
      </div>
